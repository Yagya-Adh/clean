        ...
        {data?.slice(0, 6).map((list) => (
          ...
          >
            <div className="flex flex-row place-items-center justify-between w-full h-full">
              <h3
                className="text-xl sm:text-xl xl:text-3xl text-clean-black-10 py-4 font-inter font-normal"
                onClick={() => handleQuestionToggle(list.id)}
              >
                {list.question}
              </h3>
              <span
                className="text-5xl sm:text-4xl font-inter font-light text-clean-black-10 cursor-pointer flex justify-center place-items-start"
                onClick={() => handleQuestionToggle(list.id)}
              >
                {openQuestion === list.id ? "-" : "+"}
              </span>
